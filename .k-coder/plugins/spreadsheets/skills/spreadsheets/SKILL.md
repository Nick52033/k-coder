---
name: spreadsheets
description: "Create, edit, analyze, and export local XLSX, CSV, and TSV spreadsheets in k-Coder with formulas, formatting, charts, and validation. Produces local files for Google Sheets import; does not control live Excel or connected Google Sheets."
---
...
For a net-new Google Sheets request, create and verify a local `.xlsx` first. This installation does not have a native Google Drive import connector: provide the real XLSX and explain that it can be imported into Google Sheets. Use a cloud import or browser upload only if the current tool catalog provides it and the user has authorized that action. Never invent a Sheets link or claim a local save imported a cloud document.

### Edits

For existing Google Sheets, use an explicitly available and authorized connector/browser workflow, or edit a local exported XLSX. Do not claim local edits updated the cloud original. Live Excel control is not provided by this plugin.



# Tools + Contract
- Discover the installed runtime via `run_command`: `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action info`. Execute a builder with `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action node -Script outputs/<task>/build.mjs`. No Codex dependency-loader tool is required or available in k-Coder.
- Use the already installed `@oai/artifact-tool` JS library for authoring, inspecting, recalculating, and exporting XLSX; the launcher resolves its public imports without links, junctions, or new package installs.
- Read indexed guides with `plugin_resource_read`, create builders with `apply_patch`/`write_file`, and use `run_command` under host policy for execution. All tool file paths must be workspace-relative. Keep inputs, outputs and scratch files within the workspace; generated Markdown belongs under `docs/`.
- Do not create links or junctions into external dependency directories. The runtime directory is only read to load installed programs and libraries.
...
- If the launcher reports missing dependencies, report that specific setup blocker; do not guess paths, install packages, import bundled internals, or perform a broad filesystem search.
...
- Use installed Python/openpyxl to independently reopen exported files and check formulas and cached values: `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action python -Script outputs/<task>/verify.py`. Keep artifact-tool as the authoring engine unless a concrete unsupported feature requires a documented local fallback. openpyxl alone does not calculate formulas.
...
- For nontrivial work, use the current host's `update_plan`: build, verify/render, repair meaningful issues, then finalize. Plan the source, model and summary sheets before complex analytical work.
- Current Windows runtime limitation: PNG rendering writes actual images but may exit with native code -1073740791 afterward. Preserve and independently verify exported XLSX, inspect complete preview files, and explicitly report the renderer failure; do not hide a nonzero exit or claim the rendering tool is fully healthy. Pure export and formula recalculation are separately verified.
