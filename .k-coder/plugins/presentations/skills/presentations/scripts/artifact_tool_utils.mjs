...
function defaultRuntimeNodeModules() {
  if (process.env.K_CODER_ARTIFACT_NODE_MODULES) return process.env.K_CODER_ARTIFACT_NODE_MODULES;
  throw new Error('Use scripts/plugin-artifact-runtime.ps1 -Action node to launch presentation helpers.');
...
export async function ensureArtifactToolWorkspace(workspaceDir) {
  if (process.env.PRESENTATIONS_RUNTIME === "local-granola") {
    throw new Error('The internal local-granola development runtime is not available in k-Coder.');
  }

  const resolvedWorkspace = path.resolve(workspaceDir);
  const repository = fsSync.realpathSync(process.cwd());
  const relative = path.relative(repository, resolvedWorkspace);
  if (relative.startsWith('..') || path.isAbsolute(relative)) throw new Error('Presentation workspace must remain inside the current project.');
  let cursor = resolvedWorkspace;
  while (true) {
    if (fsSync.existsSync(cursor)) {
      const stat = fsSync.lstatSync(cursor);
      if (stat.isSymbolicLink() || fsSync.realpathSync(cursor).toLowerCase() !== cursor.toLowerCase()) throw new Error('Presentation workspace links and junctions are forbidden.');
    }
...
async function loadLucide(workspaceDir) {
  const resolverPath = path.join(defaultRuntimeNodeModules(), "__lucide_resolver__.mjs");
  const requireFromRuntime = createRequire(resolverPath);
  const entrypoint = requireFromRuntime.resolve("lucide");
