---
name: presentations
description: Create and edit local PowerPoint PPTX decks in k-Coder with the installed artifact-tool runtime, preserving editable slides and verifying exported content.
---
...
Use the already installed artifact-tool runtime, including presentation JSX.
Discover it with `run_command`: `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action info`.
Execute a workspace-relative builder with `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action node -Script outputs/<task>/build.mjs`.
The launcher resolves `@oai/artifact-tool` and `@oai/artifact-tool/presentation-jsx` without installing packages or creating workspace junctions. Version 2.8.59 is verified on this machine.
Use `plugin_resource_read` for indexed guides, `apply_patch`/`write_file` for builders,
and `run_command` for execution through existing host policy. Do not call a Codex
dependency loader, imagegen, or app connector absent from the current tool catalog.
Keep all inputs, builders, scratch files and final outputs within the current workspace;
generated Markdown belongs under `docs/`. Tool file paths are workspace-relative.
The bundled guide's `node`/`python` commands below are argument examples: execute
their discovered workspace-relative script through this launcher (`-Action node`
or `-Action python`, `-ScriptArguments` for helper arguments), not a system alias.

Local verification boundary: artifact-tool can export editable PPTX; python-pptx
can reopen it and verify slide/text content. PNG rendering produces real previews,
but the current Windows native renderer can exit with code -1073740791 after writing
them. Do not ignore that process failure or label the renderer fully healthy. If it
occurs, preserve the generated PPTX, structurally reopen it, inspect any complete
preview files, and report the renderer limitation. LibreOffice is not installed.
Do not claim exact template fidelity without the required visual checks.
...
For raster images or reference comps, use user-provided or verified assets, or an
image-generation tool only when it is actually registered in the current host.
Bundled scripts may write imagegen prompt files, but must not call external
image APIs or read API keys.

Thread-scoped paths:

- `SKILL_DIR=.k-coder/plugins/presentations/skills/presentations`
- `THREAD_ID=<timestamp-plus-short-random-suffix for this task>`
...
Use workspace-relative paths in tool commands and absolute links in final handoffs. Keep all generated planning text
...
provides an external output directory, create the deliverable in the workspace
and explain the host path boundary; generated `.txt` plans, contact sheets, previews, layout files, prompt
...
is under `outputs/$THREAD_ID/presentations/`. Generate a task id with timestamp
plus a short random suffix; no Codex thread environment variable is required.
