---
name: documents
description: Create, edit, redline, and comment on local DOCX and Word documents in k-Coder, using the installed Python runtime and structural validation; render page previews when LibreOffice is available.
---

# Documents Skill (Read • Create • Edit • Redline • Comment)

Use this skill when you need to create or modify `.docx`, Word, or Google
Docs-targeted document artifacts **in the current k-Coder workspace** and verify
them visually.

## Tools + Contract

- Discover the installed runtime with `run_command`: `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action info`. Execute a workspace-relative Python builder with `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action python -Script outputs/<task>/build.py`. The launcher locates the already installed Python and docx libraries; it does not install dependencies. Do not call a Codex dependency-loader tool.
- Read this plugin's indexed reference resources with `plugin_resource_read`; edit generated builders with `apply_patch` or `write_file`. All tool paths are workspace-relative. Keep builders, inputs and outputs inside the workspace; keep generated Markdown in `docs/`. Run commands through the existing host policy and approvals. Never create links or junctions to external runtime directories.
- To run a bundled helper, use its full workspace-relative script path, for example `powershell -NoProfile -File scripts/plugin-artifact-runtime.ps1 -Action python -Script .k-coder/plugins/documents/skills/documents/render_docx.py -ScriptArguments ...`. Check the discovered helper filename and actual arguments first. The shell examples below illustrate helper arguments; execute them using this launcher instead of a system Python alias.
...
- Run any builder or helper file from the current writable workspace, not from the managed dependency directory itself.
...
For a net-new Google Docs request, create and verify a local `.docx` first. This k-Coder installation has no native Google Drive import connector. Return the actual DOCX and explain that the user can import it into Google Docs. Only use a cloud import tool if it is present in the current tool catalog and the user has authorized the upload; never invent a Google Docs link or claim a local save performed a cloud import.
...
For an explicitly requested browser upload, use only the current registered browser tools and authorized account context. Do not ask the user to install a Codex-only connector into k-Coder.
...
```bash
python render_docx.py outputs/<task>/input.docx --output_dir outputs/<task>/preview
# If debugging LibreOffice:
python render_docx.py outputs/<task>/input.docx --output_dir outputs/<task>/preview --verbose
# Optional: also write <input_stem>.pdf to --output_dir (for debugging/archival):
python render_docx.py outputs/<task>/input.docx --output_dir outputs/<task>/preview --emit_pdf
```
