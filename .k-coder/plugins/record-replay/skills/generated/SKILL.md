...
使用 browser_navigate({url}) 先到目标页面，browser_snapshot({}) 获取当前可见文本；输入使用 browser_type({selector,text})，点击使用 browser_click({selector})，最后 browser_snapshot 或 browser_screenshot 验证。原文 browser_run 是历史工具名，不可调用；ARIA/Playwright locator 示例须转换为实际 CSS 选择器，不能臆造参数。所有步骤按当前工具 Schema、用户授权和页面事实执行。

密码等凭据不得填入工作流 JSON、生成脚本、日志或聊天上下文。需要登录时由用户在真实浏览器完成或使用已授权的凭据机制；示例页面不能作为登录目标事实。缺少必要变量先澄清，不把示例占位符当作用户真实数据。
...
Resolve `keyword` from the user's request. The imported workflow's default is only
an example; replace it with the requested keyword before replay.
