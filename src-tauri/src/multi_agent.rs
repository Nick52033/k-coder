...
impl ToolHandler for AgentToolHandler {
    fn definition(&self) -> ToolDefinition {
        let (name, description, properties, required) = match self.operation {
            AgentToolOperation::Create => (
                "create_agent",
                "Create a bounded subagent only for an independently deliverable task that reduces total work. Handle routine commits, pushes, status checks and small fixes directly unless the user or project rules require delegation. Return immediately while the subagent runs in the background. Start independent delegated tasks first, then do your own non-overlapping work before waiting. Do not duplicate delegated work just to stay busy. Omit tokenBudget unless explicitly requested by the user; never invent a budget based on task size or desired answer length.",
