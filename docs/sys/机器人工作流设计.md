...
- 按 ADR 0061，`webapp-testing` 已具备 E2E/UI 节点的完整方法论正文（静态/动态决策树、侦察-行动、证据与限制、host 边界），并随包一个**本地实现**的标准库 harness `robot-pack/webapp-testing/scripts/with_server.py`：它负责启动本地 dev server、在 `127.0.0.1`/`::1` 上轮询就绪、运行一条验证命令、再清理整棵进程树。宿主从不执行该文件——模型只能经 `skill_resource_read` 读出、`write_file` 落到工作区，再用受策略、沙箱、超时与审批约束的 `run_command` 运行。Python 3.8+、`playwright` 包与 Chromium 均为用户自备前置条件，k-Coder 不安装、不下载、不打包；缺失时降级到内置 `browser_*` 工具或如实报告未覆盖的检查项。CDP 控制台捕获与页面 JS eval 仍属未完成增强，不在本轮范围。
