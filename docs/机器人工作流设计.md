...
- 工作流节点定义支持可选 `skills` 字段；节点激活时宿主读取绑定技能的 SKILL.md 正文，以 `[Workflow Skill: <name>]` 分节注入受控运行时指令，每节点最多 4 个技能、单技能 16 KiB、合计 48 KiB。
- `read` 技能直接注入正文；`write`/`external` 技能须已启用，未启用时仅注入占位声明。项目/全局来源技能缺失时跳过并审计；内置定义绑定在启动时校验，缺失则扩展运行时关闭失败。
