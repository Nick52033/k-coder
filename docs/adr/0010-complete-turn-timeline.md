...
13. 命令或自动操作进入完成态时，前端保留原有 `details` 节点和时间线子树，通过 `details::details-content` 的插值高度缓出收缩内容，避免终态切换卸载/重建造成闪烁；摘要箭头同步使用较慢的缓出过渡。`prefers-reduced-motion: reduce` 时立即收起，不改变事件顺序、持久化事实或用户手动展开能力。
