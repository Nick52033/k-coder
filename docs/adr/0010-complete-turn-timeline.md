...
7. `read_file` 的有界 `ToolResult.output` 是 Provider 历史与审计界面共享的事实。无范围参数时默认最多返回 64 KiB；`startLine/lineCount` 与兼容的 `offset/limit` 互斥，显式范围最多返回 256 KiB。前端必须原样展示已持久化结果，不能再二次截断或重新读取当前文件；代码视图按需加载只读 Monaco，并使用结果元数据恢复起始行号。
...
- 文件读取的上下文成本在工具边界确定，审计界面与模型所见保持一致；按需加载编辑器避免未展开的历史读取活动承担 Monaco 渲染成本。
