...
## 2026-09-15 修订：命令超时参数映射

P10-183 的长命令原生验收发现，公开 Schema 使用 `timeoutMs`，但 `RunCommandArguments` 原来按 Rust 字段名 `timeout_ms` 反序列化，导致模型传入的超时被忽略并落回默认 120,000 毫秒。处理器明确以 `timeoutMs` 读取，同时保留旧内部拼写的别名；省略参数仍取原默认值。公开 Schema 的 1～3,600,000 毫秒范围和执行层的再次校验保持不变，不放宽超时或取消。

新增回归以真实进程验证 `timeoutMs: 10` 会得到 `timed_out`，并覆盖公开 660,000 毫秒、默认值和旧拼写映射。长命令通过公开参数设置 660,000 毫秒，验证能越过旧 120 秒命令默认值及旧 600 秒 Turn 时间门。
