...
已接受，2026-08-22；2026-09-08 修订批次收敛与压缩后上下文重载规则；2026-09-10 修订纠偏批次生命周期与机器人节点观察域；2026-09-17 将压缩后的单批重载扩展为最多 8 批的补读窗口。

2026-09-18：本文关于重复正文抑制、纠偏后单文件硬停及固定补读额度的决定已由 [ADR 0060](0060-nonfatal-read-observations.md) 取代。下文保留为历史设计记录；当前运行时成功读取返回正文，重复提示不单独终止 Turn。
...
2026-09-10 的全栈机器人会话 `583d9226-b229-41b0-9138-39c702558f19`、Turn `37702baa-9cdb-4c6d-b052-90165b1839f7` 又暴露出纠偏生命周期过长。该 Turn 跨 8 个工作流节点运行了 121 次 Provider 请求、334 个工具结果和 6 次 Compaction。代码审查节点对 `server/src/models/db.ts` 的一次恢复纠偏已进入下一请求，模型在该请求中改为检查构建产物和其他文件；经过后续测试、构建、安全扫描及 Compaction 后，模型重新读取源码，运行时仍以“历史上送达过纠偏”的布尔状态直接硬停。临时 System 纠偏只存在于一个请求，不能约束已经推进到后续工作或新机器人节点的任意未来批次。

## 决策

2026-09-17 的生产会话 `aa685952-3f16-45c0-8b7b-10bef073f5e7` 表明：摘要保留读取来源不等于模型仍拥有全文。`SsrRecordMain.cs` 在压缩后的五次分段补读最终被旧单批窗口误判为空转。因此将一次性窗口扩展至最多 8 个 Provider 批次；已开始重载后再次 Compaction 会关闭该窗口，未用完的额度不结转，也不重新分配。
...
3. `read_observation_recovery_required` 为下一次 Provider 请求排队一条带路径/修订身份的宿主 System 纠偏。纠偏携带 JSON 编码的路径、修订和行范围，要求复用已有观察，并禁止通过 `read_file`、Shell 或仓库搜索绕过同一修订的重读边界。该纠偏只作用于下一次请求，最多同时排队 4 条，不写入 JSONL，也不伪造成 User 消息；运行时只有在纠偏文本实际加入 Provider 请求后，才记录对应路径/修订及该请求的 Provider 批次号。
4. 如果 Provider 在携带纠偏的同一批次仍对该修订发起高度重叠重读，运行时返回失败的 `repeated_observation_loop` 并终止 Turn。硬停止必须同时匹配路径、修订和纠偏送达批次；模型在纠偏批次推进其他有效工作后，未来批次的重叠读取重新返回 `read_observation_recovery_required` 并获得新的单批纠偏，不能被旧批次状态直接硬停。同一响应中的后一项调用仍不能消耗尚未发生的恢复机会。失败原因使用面向用户的中文说明；结构化 `TurnError` 使用同名 code、`Tool` category 和 `retryable = true`。
...
8. Compaction 成功持久化时，运行时把当前观察域已跟踪的路径/修订标记为可重载。该修订后续最多 8 个高度重叠 Provider 响应批次允许重新返回真实正文，不执行重复正文抑制，并在持久化结果元数据中写入 `observationStatus = read_observation_rehydrated_after_compaction`、`rehydratedAfterCompaction = true` 和 `contentSuppressed = false`。同一批次内的多个重叠读取共享一个额度；额度耗尽后恢复普通重复读取保护。同一路径/修订在当前观察域内只获得这一段重载窗口，重复自动或手动续跑 Compaction 不得刷新额度。新文件修订继续建立独立状态。
9. 普通 Turn 的观察域仍覆盖整个 Turn。内置机器人成功执行宿主 `complete_workflow_node` 时，下一节点具有新的交付目标和节点级 Skill 正文，运行时清空上一节点的读取覆盖与尚未送达的纠偏；节点完成失败时不得清空。机器人定义最多 8 个节点，这个边界不会成为无界重读旁路，同一节点内继续执行上述批次收敛和硬停止。

## 安全与边界

- 来源头只使用工具处理器持久化的结果元数据。模型参数中的路径、修订或权限字段不能替代该事实，也不能扩大工作区访问能力。
- 注入 System 纠偏前，路径和修订使用 JSON 字符串编码，避免特殊文件名改变指令结构。
- 每条恢复纠偏只允许其对应 Provider 批次再获得一次决策机会，不重新执行写入，不自动批准工具，也不改变 Shell、审批、取消和进程树策略。
- 压缩后重载仍逐次执行原始 `read_file` 的路径规范化、工作区逃逸、文件大小、编码、范围和输出上限检查；它只改变成功结果是否抑制正文，不缓存文件、不扩大范围，也不接受模型提供的修订作为授权事实。
- 重载额度绑定当前观察域内宿主观察到的路径/修订状态，并以“每修订至多 8 个 Provider 批次”为生命周期上限；重复 Compaction 不能把它变成无限重读旁路。只有宿主确认成功的机器人节点转换才能开始新观察域。
...
Compaction 后摘要只有来源而缺少实现正文时，Provider 可以在一个有界重载窗口内重新取得该修订的多个正文范围，而不会被过时的内存覆盖信息永久阻断。代价是每个已观察文件修订在当前观察域中最多增加 8 个重载批次的上下文和读取成本；重复压缩不增加额度。

纠偏不再永久污染同一 Turn 的后续工作；只有携带纠偏的请求立即违反边界才会硬停。机器人进入新的有界节点后可以为该节点的独立目标重新检查源码，单节点内的重复保护保持不变。跨批次反复重读仍受恢复纠偏和全局无进展窗口共同约束。
...
- 批次回归覆盖同一 Provider 响应中的多个重叠范围只推进一次状态，达到 `RecoveryRequired` 后同批后一项不能触发硬停止；只有携带纠偏的下一批次仍重读才进入 `RepeatedLoop`，纠偏批次执行其他有效工作后不得让未来批次继承硬停止。
- Compaction 回归覆盖“压缩后继续”的多个重叠批次返回真实正文和重载元数据，并覆盖第二次 Compaction 不重新开放同一路径/修订额度。
- 补读边界回归覆盖生产五段范围、同批多个范围共享额度、第 8 批返回正文、第 9 批恢复抑制、额度耗尽后纠偏及硬停止，以及文件修订变化后建立独立窗口。
...
- 机器人节点回归覆盖成功 `complete_workflow_node` 清空覆盖和待发送纠偏、失败完成保持原观察域，以及下一节点首次读取同一修订重新返回正文。
